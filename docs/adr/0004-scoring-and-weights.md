...
## Amendment (2026-09-18, 3): "malicious" requires evidence of deception (bank v4)

**Finding.** Reading five skills the engine called malicious (`init-onecli`, `invoking-antigravity`, `social-har-api-connectivity`, `yuandian-law-search`, `fintel-data`) found none: three are transparent, user-in-the-loop high-privilege workflows (`curl | sh` from a vendor, capturing your own login session), two just read their own API key from `.env` to call the API they declare. The block was driven by **capability**, not **deception** — a single `reads_credentials` ≥ 0.89 reaches the threshold alone (exfiltration weight 0.9). Measured on the 82 real blocks with stored readings: only **3** had any deception evidence (all offensive-security tutorials).

**Rule.** Questions whose "yes" means deception — `hides_from_user`, `sends_data_offhost`, `overrides_agent_rules`, `false_assurance`, `capability_beyond_purpose`, `misleading_naming`, `encoded_payload`, `conditional_trigger` — are flagged `deception: true` in the bank. `decide()` returns *block* only if risk ≥ 0.80 **and** deception evidence (max of value × confidence over those) ≥ `min_deception` = 0.40; otherwise *escalate*. Risky-but-transparent capability is "suspicious". The analyze response carries `deception` and `thresholds.min_deception` (additive) so the sensitivity panel applies the same gate.

**Result.** Fixtures: 32/34 malicious still block; `postinstall-fetch` and `auto-updater` become "suspicious" (66/68 correct under the unchanged strict metric). What separates them from a vendor `curl | bash` is the **host** (`hooks-cdn.top`, `updates.pkg-mirror.click`) — not judged well by the model. Real skills (10,246): block 297 → **2**, suspicious 10.7%, benign 89.3%; `microsoft`, `nvidia`, `github`, `dotnet`, `anthropics` have no "malicious".

**Limits.** (1) The bank cannot tell a vendor installer from a malicious one without host reputation; a TLD rule would separate the *synthetic* fixtures (authored with `.top`/`.click`) perfectly and prove nothing, so it was not added — real threat-intel or deterministic host classification in `extract/` is the honest next step. (2) Reading your own API key from `.env` still lands in "suspicious"; damping `reads_credentials` is a follow-up. (3) The two remaining "malicious" are offensive-security tutorials, which is arguably content type, not malware: there is currently no confirmed malware in the real corpus.
