...
- **Verdict names:** the UI says benign / suspicious / malicious; the contract, engine and DB keep allow / escalate / block (`VERDICT_LABEL`). Don't rename the internal values. Thresholds: block 0.80, review 0.55 (ADR-0004 amendment); `decide()` in `score.py` is the single rule.
- **Filling the catalog** (worker, from `apps/worker`): `worker ingest-topics` (crawl top repos per GitHub topic; needs `GITHUB_TOKEN`) → `worker run --forever --concurrency 16 --max-usd-day 5` (daily spend cap shared through the DB) → `worker rethreshold` after any threshold change (0 API calls). Skills only — no MCP sources. Real cost ≈ $0.00027/skill.
